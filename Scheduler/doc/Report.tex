...
In the default scenario the MPD is activated and all the parameters are left untouched. This means that also if there are 20 clients in the system , in the active part only 10 will be admitted because the reserve station will prevent them to go any further, until a process will exit from the swap out.
...
This will reduce the size of the queues in the system, 
\begin{table}[!ht]
    \centering
