...
The four cases presented previously simulate the system with 2 different types of jobs (interactive and batch) with 2 types of queues (FCFS and Round Robin). This hypothetical cases are in reality possible in modern operative systems, also if in this simulator there are some simplifications. For example, the CPU doesn't consider in any way the cost (in terms of time) of a context switch, cost that will show that for example the case in which we use an FCFS queue for the CPU with Batch processes could have a slightly better performance that if a Round Robin queue is used in the same situation, because the CPU is avoiding the context switch delay, hoverer usually the context switch time is negligible in moderns CPU since it is orders of magnitude lower respect to the context switch time (nanoseconds vs milliseconds) . In this simulation instead they have comparable performance in terms of mean clients at the CPU, in conclusion the difference in terms of performance when Batch processes (negative exponential distribution of service times at the CPU) are considered is minimal when FCFS queue or Round Robin one are used for the CPU. 
\\
A very different situation happen when also interactive processes are considered. In this case the role of the time slice is fundamental for the system to work properly, because when it's disabled the CPU tend to accumulate process in the queue and to strongly increase the waiting times for all those processes, this is due to those process that have an high service time and that must be completed when the CPU has an FCFS queue. This behavior can lead to trashing the system very quickly in case the multi programming degree is increased, the user would enqueue more processes but the throughput tend to lower instead of increasing, demonstrating the importance of the time slice mechanism (the use of the Round Robin queue) when the kernel should manage interactive processes (and so a user interacting with the OS in some way).\\
