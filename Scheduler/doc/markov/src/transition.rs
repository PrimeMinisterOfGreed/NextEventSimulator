...
use crate::{parameters::Params, state::{State, StateDescriptor}};
...
impl Transition {
    ...
    pub fn detect(&mut self){
        ...
        match argmin {
            ...
            StateDescriptor::Nio1 => {
                if argmax == StateDescriptor::Ncpu{self.t_type = TransitionType::Io1ToCpu}
            },
            StateDescriptor::Nio2 => {
                if argmax == StateDescriptor::Ncpu{self.t_type = TransitionType::Io2ToCpu}
            },
        }
    }
    ...
    fn cpu_onarrive_stage_selector(&self) -> f64{
        if self.head.n_cpu > 0 {return 1.0}
        else if self.tail.cpu_stage == 1 {return Params::instance().alpha}
        else if self.tail.cpu_stage == 2 {return Params::instance().beta}
        panic!("invalid cpu stage")
    }

    fn cpu_onleave_stage_selector(&self) -> f64{
        if self.tail.n_cpu == 0 {return 1.0}
        else if self.tail.cpu_stage == 1 {return Params::instance().alpha}
        else if self.tail.cpu_stage == 2 {return Params::instance().beta}
        panic!("invalid cpu stage")
    }
    ...
    fn cpu_to_io(&self) -> f64{
        self.cpu_leave()* if self.t_type == TransitionType::CpuToIo1{
    ...
    fn io_to_cpu(&self) -> f64{
        ...
        service*self.cpu_onarrive_stage_selector()
    }
